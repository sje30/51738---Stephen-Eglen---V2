...
\url{https://authors.acm.org/author-services/publons}).  \rev{As well
  as this, ACM in August 2020 provided badges to indicate when code is
  available if it can generate the same results as in the paper.  In
  principle, these basdges could be awarded by a CODECHECKER too.}
...
journals.  \rev{Finally, the \textit{Journal of Open Science Software}
...
effectively providing another form of check.}
...
compromise that should be reconsidered.  \rev{Likewise, if a research
requires specific hardware, e.g. GPU, we are reliant on the
codechecker having access to similar hardware, which adds another barrier.}
